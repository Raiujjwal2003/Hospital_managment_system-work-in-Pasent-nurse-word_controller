...
class Admin extends CI_Controller
{
    ...
    function ward($task = "", $ward_id = "")
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase(' ward_add_successfuly'));
            redirect(site_url('admin/ward'), 'refresh');
        }
        
        if ($task == "update") {
            $this->crud_model->update_ward_info($ward_id);
            $this->session->set_flashdata('message', get_phrase('no_of_beds_updated_successfuly'));
            redirect(site_url('admin/ward'), 'refresh');
        }
    ...
    function wardtype($task = "", $wardtype_id = "")
    {
        if ($this->session->userdata('admin_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url(), 'refresh');
        ...
        if ($task == "create") {
            $this->crud_model->save_wardtype_info();
            $this->session->set_flashdata('message', get_phrase(' ward_allortment_successfuly'));
            redirect(site_url('admin/wardtype'), 'refresh');
        }
        
        if ($task == "update") {
            $this->crud_model->update_wardtype_info($wardtype_id);
            $this->session->set_flashdata('message', get_phrase('wardtype_updated_successfuly'));
            redirect(site_url('admin/wardtype'), 'refresh');
        }
        
        if ($task == "delete") {
            $this->crud_model->delete_wardtype_info($wardtype_id);
            redirect(site_url('admin/wardtype'), 'refresh');
        }
        
        $data['wardtype_info']       =   $this->crud_model->select_wardtype_info();
        $data['page_name']       = 'manage_wardtype';
        $data['page_title']      = get_phrase('wardtype');
        $this->load->view('backend/index', $data);
    }
    ...
    function name($task="" ){
        ...
        if ($task == "create") {
            $this->crud_model->save_name_info();
            $this->session->set_flashdata('message', get_phrase(' name_add_successfuly'));
            redirect(site_url('admin/name'), 'refresh');
        }
        



        $data['name_info']       = $this->crud_model->select_name_info();
        $data['page_name']       = 'add_name';
        $data['page_title']      = get_phrase('name');
        $this->load->view('backend/index', $data);
    ...
    function compounder($task= "", $compounder_id = "")
    {
        if ($this->session->userdata('admin_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url(), 'refresh');
        ...
        if ($task == "create") {
            $this->crud_model->save_compounder_info();
            $this->session->set_flashdata('message', get_phrase('compounder_info_saved_successfuly'));
            redirect(site_url('admin/compounder'), 'refresh');
        }
        
        if ($task == "update") {
            $this->crud_model->update_compounder_info($compounder_id);
            redirect(site_url('admin/compounder'), 'refresh');
        }
        
        if ($task == "delete") {
            $this->crud_model->delete_compounder_info($compounder_id);
            redirect(site_url('admin/compounder'), 'refresh');
        }
        
        $data['compounder_info'] = $this->crud_model->select_compounder_info();
        $data['page_name']  = 'manage_compounder';
        $data['page_title'] = get_phrase('compounder');
        $this->load->view('backend/index', $data);
    }
    ...
    // function compounder($task="", compounder_id=""){
    //     if($this->session->userdata('amdin_page') != 1){
    //         $this->session->set_userdata('last_page'current_url());
    //         redirect(site_url(),'refresh');
    //     }
