...
class Login extends CI_Controller
{
    ...
    public function index()
    {
        ...
        else if ($this->session->userdata('compounder_login') == 1)
            redirect(site_url('compounder'), 'refresh');
    ...
    function validate_login($email = '', $password = '')
    {
        ...
        $query = $this->db->get_where('compounder', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('compounder_login', '1');
            $this->session->set_userdata('login_user_id', $row->compounder_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'compounder');
            return 'success';
        }
        ...
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('laboratorist_log9in', '1');
    ...
    function reset_password()
    {
        ...
        // checking credential for compounder
        $query = $this->db->get_where('compounder', array(
            'email' => $email
        ));
        if ($query->num_rows() > 0) {
            $reset_account_type = 'compounder';
            $this->db->where('email', $email);
            $this->db->update('compounder', array(
                'password' => sha1($new_password)
            ));
        }
