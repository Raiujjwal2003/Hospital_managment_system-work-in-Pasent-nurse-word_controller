...
class compounder extends CI_Controller
{
    ...
    function index()
    {
        if ($this->session->userdata('compounder_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url(), 'refresh');
        }
        
        $data['page_name']  = 'dashboard';
        $data['page_title'] = get_phrase('compounder_dashboard');
    ...
    function patient($task = "", $patient_id = "")
    {
        ...
        if ($task == "create") {
            ...
            redirect(site_url('compounder/patient'), 'refresh');
        }
    ...
    function bed($task = "", $bed_id = "")
    {
        ...
        if ($task == "create") {
            ...
            redirect(site_url('compounder/bed'), 'refresh');
        }
    ...
    function bed_allotment($task = "", $bed_allotment_id = "")
    {
        ...
        if ($task == "create") {
            ...
            redirect(site_url('compounder/bed_allotment'), 'refresh');
        }
    ...
    function blood_bank($task = "", $blood_group_id = "")
    {
        ...
        if ($task == "update") {
            ...
            redirect(site_url('compounder/blood_bank'), 'refresh');
        }
    ...
    function blood_donor($task = "", $blood_donor_id = "")
    {
        ...
        if ($task == "create") {
            ...
            redirect(site_url('compounder/blood_donor'), 'refresh');
        }
    ...
    function report($task = "", $report_id = "", $param3 = '')
    {
        ...
        if ($task == "create") {
            ...
            redirect(site_url('compounder/report'), 'refresh');
        }
    ...
    function manage_profile($task = "")
    {
        ...
        $compounder_id = $this->session->userdata('login_user_id');
        if ($task == "update") {
            $this->crud_model->update_compounder_info($compounder_id);
            redirect(site_url('compounder/manage_profile'), 'refresh');
        }
        
        if ($task == "change_password") {
            $password             = $this->db->get_where('compounder', array(
            ...
            if ($password == $old_password && $new_password == $confirm_new_password) {
                ...
                $this->db->update('compounder', $data);
