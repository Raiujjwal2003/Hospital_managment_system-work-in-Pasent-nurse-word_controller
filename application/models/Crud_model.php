...
class Crud_model extends CI_Model {
    ...
    function select_wardtype_info()
    {
        return $this->db->get('wardtype')->result_array();
        
    }


    function save_wardtype_info()
    {
        $data['name']  = $this->input->post('name');
        $data['description'] = $this->input->post('description');
        $data['no_of_beds'] = $this->input->post('no_of_beds');
        $returned_array        = null_checking($data);
        $this->db->insert('wardtype',$returned_array);

        $patient_id = $this->db->insert_id();
        move_uploaded_file($_FILES['dept_icon']['tmp_name'], 'uploads/frontend/department_images/'. $patient_id.'.png');
    }


    function update_wardtype_info($wardtype_id)
    {
        $data['name'] 		= $this->input->post('name');
        $data['description'] 	= $this->input->post('description');
        $returned_array = null_checking($data);
        $this->db->where('wardtype_id',$wardtype_id);
        $this->db->update('wardtype',$returned_array);
        move_uploaded_file($_FILES['dept_icon']['tmp_name'], 'uploads/frontend/department_images/'. $department_id.'.png');
    }


    
    function delete_wardtype_info($wardtype_id)
    {
        if (file_exists(base_url('uploads/frontend/department_images/'.$wardtype_id.'.png'))) {
            unlink(base_url('uploads/frontend/department_images/'.$wadtype_id.'.png'));
        }
        $this->db->where('wardtype_id',$wardtype_id);
        $this->db->delete('wardtype');
    }


    function select_name_info()
    {
    ...
    function save_name_info()
    {
        $data['first_name']  = $this->input->post('first_name');
        $data['last_name'] = $this->input->post('last_name');
        $returned_array        = null_checking($data);
        $this->db->insert('name',$returned_array);
    ...
    function update_ward_info($ward_id)
    {
        ...
        $this->db->update('ward',$returned_array);
        move_uploaded_file($_FILES['dept_icon']['tmp_name'], 'uploads/frontend/department_images/'. $department_id.'.png');
    }

    function delete_ward_info($ward_id)
    {
        if (file_exists(base_url('uploads/frontend/ward_images/'.$ward_id.'.png'))) {
            unlink(base_url('uploads/frontend/ward_images/'.$ward_id.'.png'));
        }
        $this->db->where('ward_id',$ward_id);
        $this->db->delete('ward');
    }
    ...
    function save_ward_info()
    {
        ...
        $data['no_of_beds'] = $this->input->post('no_of_beds');
        $returned_array        = null_checking($data);
        $this->db->insert('ward',$returned_array);
    ...
    function select_compounder_info()
    {
        return $this->db->get('compounder')->result_array();
    }

    function save_compounder_info()
    {
        $data['name'] 		= $this->input->post('name');
        $data['email'] 		= $this->input->post('email');
        $data['password']       = sha1($this->input->post('password'));
        ...
        if ($validation == 1) {
          $returned_array = null_checking($data);
          $this->db->insert('compounder',$returned_array);
          $compounder_id  =   $this->db->insert_id();
		  $this->email_model->account_opening_email('compounder', $data['email'], $this->input->post('password'));
          move_uploaded_file($_FILES["image"]["tmp_name"], 'uploads/compounder_image/' . $compounder_id . '.jpg');
        }
        else{
          $this->session->set_flashdata('error_message', get_phrase('duplicate_email'));
          redirect(site_url('admin/compounder'), 'refresh');
        }

    }

    function update_compounder_info($compounder_id)
    {
        $data['name'] 		= $this->input->post('name');
        $data['email'] 		= $this->input->post('email');
        $data['address'] 	= $this->input->post('address');
        ...
        $validation = email_validation_on_edit($data['email'], $compounder_id, 'compounder');
        if ($validation == 1) {
          $returned_array = null_checking($data);
          $this->db->where('compounder_id',$compounder_id);
          $this->db->update('compounder',$returned_array);
          move_uploaded_file($_FILES["image"]["tmp_name"], 'uploads/compounder_image/' . $compounder_id . '.jpg');
          $this->session->set_flashdata('message', get_phrase('updated_successfuly'));
        }
        else{
    ...
    function delete_compounder_info($compounder_id)
    {
        $this->db->where('compounder_id',$compounder_id);
        $this->db->delete('compounder');
    }
